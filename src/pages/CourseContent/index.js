...
function CourseContent(props) {
  ...
  const { courseContent: { loading, data }, selectedExercise  } = useSelector(({ Course }) => Course)
  ...
  const handleBackClick = useCallback(() => {
    ...
    if(subIndex === 0 || subIndex) {
      if (subIndex > 0) {
        const childExerciseList = get(exercise, `parentExercise.childExercises[${selectedIndex}].childExercises`)
      //  return  handleExerciseChange({...childExerciseList[subIndex-1], parentExercise: exercise.parentExercise}, selectedIndex, subIndex - 1)
      } else if(subIndex === 0) {
