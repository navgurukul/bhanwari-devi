...
const GoBackArrow  = (props) => {
  ...
  const handleBackClick = () => {
    ...
    if(isFirstChildSelected) {
      ...
      const selectedChildExercise = mainExercise.childExercises[previousSubExerciseIndex]
      const selectedExerciseInfo = { exercise: selectedChildExercise, parentExercise: mainExercise, index, subExerciseIndex: previousSubExerciseIndex }
      dispatch(courseActions.updateSelectedExercise(selectedExerciseInfo))
    } else {
      const previousExerciseIndex = index - 1
      const previousExercise = get(data, `exerciseList[${previousExerciseIndex}]`)
      ...
      if(childExercises.length) {
        ...
        selectedMainExercise = { exercise: childExercises[lastChildExerciseIndex], parentExercise: previousExercise, index: previousExerciseIndex, subExerciseIndex: lastChildExerciseIndex }
      } else {
        selectedMainExercise = { exercise: previousExercise, index: previousExerciseIndex }
      }
      dispatch(courseActions.updateSelectedExercise(selectedMainExercise))
    }
  }
